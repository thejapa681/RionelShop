      ...
      <div className="hidden border-b border-border bg-primary/10 md:block">
        <div className="mx-auto flex h-8 max-w-7xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-4">
            <Link href="/suporte" className="text-xs text-muted-foreground hover:text-foreground">
              Central de Ajuda
            </Link>
            <Link href="/rastreio" className="text-xs text-muted-foreground hover:text-foreground">
              Rastrear Pedido
            </Link>
          </div>
        </div>
        ...
        <div className="flex h-16 items-center gap-4">
          ...
          <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
            ...
            <SheetContent side="left" className="w-80 bg-card">
              <SheetHeader>
                <SheetTitle className="gradient-text text-left text-2xl font-bold">Rionel</SheetTitle>
              </SheetHeader>
                ...
                <DropdownMenuContent align="end" className="w-80 bg-card">
                  ...
                  {notifications.length > 0 ? (
                    notifications.map((notif) => (
                      <DropdownMenuItem
                        key={notif.id}
                        className={`flex flex-col items-start gap-1 p-3 ${!notif.is_read ? "bg-primary/5" : ""}`}
                      >
